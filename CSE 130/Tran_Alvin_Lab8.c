...
Updated again to utilize files to save and load phonebooks.
*/
	...
	do{
		...
		switch(choice){	
			...
			printf("Are you sure you wish to clear ALL entries? Type \"Yes\" if so.\n");
				...
				break;
			case 8: //Load File====================================================================== Default path is same folder as program
				printf("Input file name or location for load. Input 0 for default\n");
				scanf("%s",&input);
				FILE *pRead;
				if(strcmp(input, "0") == 0) /// default is a siple write file
					strcpy(input,"Lab8Read.txt");
				pRead = fopen(input, "r");
				if(pRead == NULL)
					printf("File can't be opened. Please try again\n");
				else {
					i = 0;
					while(fgets(input, 100, pRead) != NULL){
						full = true;
						for(i = 0; i < maxEntry; i++){ //Looking for next empty spot
							if(strlen(phoneBook[i].first ) < 1 ){ //We found an empty spot
								nextEntry = i;
						...
						char *parse = strchr(input, '\n');
						if (parse)
							*parse = 0;
						
						parse = strtok(input, " ");
						strcpy(phoneBook[nextEntry].first, parse);
						parse = strtok(NULL, " ");
						strcpy(phoneBook[nextEntry].last, parse);
						parse = strtok(NULL, " ");
						strcpy(phoneBook[nextEntry].number, parse);
				...
				fclose(pRead);
				break;
			case 9: //Save file=====================================================================  Default path is same folder as program
				printf("Input file name or location for saving. This will overwrite file. Input 0 for default\n");
				scanf("%s",&input);
				if(strcmp(input, "0") == 0) /// default is a siple write file
					strcpy(input,"Lab8Write.txt");
				
				pWrite = fopen(input, "w");
